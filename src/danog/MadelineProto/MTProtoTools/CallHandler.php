    ...
    public function method_call($method, $args = [], $aargs = ['message_id' => null, 'heavy' => false])
    {
        ...
        for ($count = 1; $count <= $this->settings['max_tries']['query']; $count++) {
            try {
                ...
                $this->datacenter->sockets[$aargs['datacenter']]->outgoing_messages['a'.$message_id]['content'] = ['method' => $method, 'args' => $args];
                $this->datacenter->sockets[$aargs['datacenter']]->new_outgoing['a'.$message_id] = ['msg_id' => $message_id, 'method' => $method, 'type' => $type];
                $res_count = 0;
                $server_answer = null;
                $update_count = 0;
                $only_updates = false;
                while ($server_answer === null && $res_count++ < $this->settings['max_tries']['response'] + 1) { // Loop until we get a response, loop for a max of $this->settings['max_tries']['response'] times
                    try {
                        ...
                        //sleep(2);
                        $this->start_threads();
                        if (!isset($this->datacenter->sockets[$aargs['datacenter']]->outgoing_messages['a'.$message_id]['response']) || !isset($this->datacenter->sockets[$aargs['datacenter']]->incoming_messages['a'.$this->datacenter->sockets[$aargs['datacenter']]->outgoing_messages['a'.$message_id]['response']]['content'])) { // Checks if I have received the response to the called method, if not continue looping
                            if ($only_updates) {
                                if ($update_count > 50) {
                                    $update_count = 0;
                                } else {
                        ...
                        if (!$this->threads && !$this->run_workers) {
                            ...
                            while ($this->datacenter->sockets[$aargs['datacenter']]->reading);
                        }
                    ...
                    case 'bad_msg_notification':
                        switch ($server_answer['error_code']) {
                            ...
                            case 17:
                                ...
                                $this->datacenter->sockets[$aargs['datacenter']]->timedelta = (int) ((new \phpseclib\Math\BigInteger(strrev($this->datacenter->sockets[$aargs['datacenter']]->outgoing_messages['a'.$message_id]['response']), 256))->bitwise_rightShift(32)->subtract(new \phpseclib\Math\BigInteger(time()))->toString());
                ...
                if (isset($aargs['heavy']) && $aargs['heavy'] && isset($message_id)) {
                    //$this->datacenter->sockets[$aargs['datacenter']]->outgoing_messages['a'.$message_id]['args'] = [];
                    $this->datacenter->sockets[$aargs['datacenter']]->outgoing_messages['a'.$message_id] = [];
                    unset($this->datacenter->sockets[$aargs['datacenter']]->new_outgoing['a'.$message_id]);
                }
                if (isset($message_id) && $method === 'req_pq') {
                    unset($this->datacenter->sockets[$aargs['datacenter']]->outgoing_messages['a'.$message_id]);
                    unset($this->datacenter->sockets[$aargs['datacenter']]->new_outgoing['a'.$message_id]);
                }
                if ($canunset) {
                    $this->getting_state = false;
    ...
    public function object_call($object, $args = [], $aargs = ['message_id' => null, 'heavy' => false])
    {
        ...
        for ($count = 1; $count <= $this->settings['max_tries']['query']; $count++) {
            try {
                ...
                if ($object !== 'msgs_ack') {
                    $this->datacenter->sockets[$aargs['datacenter']]->outgoing_messages['a'.$message_id]['content'] = ['method' => $object, 'args' => $args];
                }
            } catch (Exception $e) {
