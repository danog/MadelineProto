...
trait SeqNoHandler
{
    ...
    {
        if (isset($this->datacenter->sockets[$datacenter]->incoming_messages['a'.$current_msg_id]['seq_no']) && ($seq_no = $this->generate_in_seq_no($datacenter, $this->content_related($this->datacenter->sockets[$datacenter]->incoming_messages['a'.$current_msg_id]['content']))) !== $this->datacenter->sockets[$datacenter]->incoming_messages['a'.$current_msg_id]['seq_no']) {
            \danog\MadelineProto\Logger::log(['SECURITY WARNING: Seqno mismatch (should be '.$seq_no.', is '.$this->datacenter->sockets[$datacenter]->incoming_messages['a'.$current_msg_id]['seq_no'].', '.$this->datacenter->sockets[$datacenter]->incoming_messages['a'.$current_msg_id]['content']['_'].')'], \danog\MadelineProto\Logger::ERROR);
        }
    }
