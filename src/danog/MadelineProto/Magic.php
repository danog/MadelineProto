...
class Magic
{
    ...
    /**
     * Watcher IDs of signal handlers.
     *
     * @var array
     */
    ...
    public static function class_exists()
    {
        ...
        if (!self::$inited) {
            ...
            if (\defined('SIGINT')) {
                ...
                self::$watchers []= Loop::onSignal(SIGINT, static function () {
                    Logger::log('Got sigint', Logger::FATAL_ERROR);
                    Magic::shutdown();
                });
                self::$watchers []= Loop::onSignal(SIGTERM, static function () {
    ...
    {
        self::$signaled = true;
        foreach (self::$watchers as $watcherId) {
