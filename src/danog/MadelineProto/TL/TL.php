...
trait TL
{
    ...
    {
        \danog\MadelineProto\Logger::log([$lang[$current_lang]["TL_loading"]], \danog\MadelineProto\Logger::VERBOSE);
        ...
        foreach ($files as $scheme_type => $file) {
            \danog\MadelineProto\Logger::log([sprintf($lang[$current_lang]["file_parsing"], basename($file))], \danog\MadelineProto\Logger::VERBOSE);
                ...
                foreach ($tl_file as $line) {
                    ...
                    if (preg_match('/^[^\s]+#/', $line)) {
                        ...
                        if ($id !== $nid && $scheme_type !== 'botAPI') {
                            \danog\MadelineProto\Logger::log([sprintf($lang[$current_lang]["crc32_mismatch"], $id, $nid, $line)], \danog\MadelineProto\Logger::ERROR);
                        }
                        $id = $nid;
                    }
                    if (!is_null($e)) {
            ...
            \danog\MadelineProto\Logger::log([$lang[$current_lang]["translating_obj"]], \danog\MadelineProto\Logger::ULTRA_VERBOSE);
            ...
            \danog\MadelineProto\Logger::log([$lang[$current_lang]["translating_methods"]], \danog\MadelineProto\Logger::ULTRA_VERBOSE);
    ...
    public function deserialize($stream, $type = ['type' => ''])
    {
        ...
        switch ($type['type']) {
            ...
            case 'bytes':
                ...
                if (!is_string($x)) {
                    throw new Exception($lang[$current_lang]["deserialize_not_str"]);
                }
            ...
            case 'Vector t':
                ...
                switch ($constructorData['predicate']) {
                    ...
                    case 'vector':
                        break;
                    default:
                        throw new Exception($lang[$current_lang]["vector_invalid"].$constructorData['predicate']);
                }
            case 'vector':
        ...
        foreach ($constructorData['params'] as $arg) {
            ...
            if ($arg['name'] === 'random_bytes') {
                if (strlen($x[$arg['name']]) < 15) {
                    throw new \danog\MadelineProto\SecurityException($lang[$current_lang]["rand_bytes_too_small"]);
                } else {
                    unset($x[$arg['name']]);
                }
            }
