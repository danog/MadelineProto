...
class SocketReader extends \Threaded implements \Collectable
{
    ...
    {
        \danog\MadelineProto\Logger::log([$lang[$current_lang]["shutting_down_reader_pool"].$this->current], \danog\MadelineProto\Logger::NOTICE);
    }
    ...
    public function run()
    {
        ...
        while ($number = $handler_pool->collect()) {
            \danog\MadelineProto\Logger::log([sprintf($lang[$current_lang]["shutting_down_handler_pool"], $this->current, $number)], \danog\MadelineProto\Logger::NOTICE);
        }
        $this->setGarbage();
    }
