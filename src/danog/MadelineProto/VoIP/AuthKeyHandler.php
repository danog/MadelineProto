...
trait AuthKeyHandler
{
    ...
    public function request_call($user)
    {
        if (!class_exists('\danog\MadelineProto\VoIP')) {
            throw new \danog\MadelineProto\Exception($lang[$current_lang]["libtgvoip_required"]);
        }
        ...
        if (!isset($user['InputUser']) || $user['InputUser']['_'] === 'inputUserSelf') {
            throw new \danog\MadelineProto\Exception($lang[$current_lang]["peer_not_in_db"]);
        }
        $user = $user['InputUser'];
        \danog\MadelineProto\Logger::log([sprintf($lang[$current_lang]["calling_user"], $user['user_id'])], \danog\MadelineProto\Logger::VERBOSE);
        $dh_config = $this->get_dh_config();
        \danog\MadelineProto\Logger::log([$lang[$current_lang]["generating_a"]], \danog\MadelineProto\Logger::VERBOSE);
        $a = \phpseclib\Math\BigInteger::randomRange($this->two, $dh_config['p']->subtract($this->two));
        \danog\MadelineProto\Logger::log([$lang[$current_lang]["generating_g_a"]], \danog\MadelineProto\Logger::VERBOSE);
    ...
    public function accept_call($call)
    {
        ...
        if ($this->call_status($call['id']) !== \danog\MadelineProto\VoIP::CALL_STATE_ACCEPTED) {
            \danog\MadelineProto\Logger::log([sprintf($lang[$current_lang]["call_error_1"], $call['id'])]);

            return false;
        }
        \danog\MadelineProto\Logger::log([sprintf($lang[$current_lang]["accepting_call"], $this->calls[$call['id']]->getOtherID())], \danog\MadelineProto\Logger::VERBOSE);

        $dh_config = $this->get_dh_config();
        \danog\MadelineProto\Logger::log([$lang[$current_lang]["generating_b"]], \danog\MadelineProto\Logger::VERBOSE);
        ...
        try {
            ...
            if ($e->rpc === 'CALL_ALREADY_ACCEPTED') {
                \danog\MadelineProto\Logger::log([sprintf($lang[$current_lang]["call_already_accepted"], $call['id'])]);

                return true;
            }
            if ($e->rpc === 'CALL_ALREADY_DECLINED') {
                \danog\MadelineProto\Logger::log([sprintf($lang[$current_lang]["call_already_declined"], $call['id'])]);
    ...
    public function confirm_call($params)
    {
        ...
        if ($this->call_status($params['id']) !== \danog\MadelineProto\VoIP::CALL_STATE_REQUESTED) {
            \danog\MadelineProto\Logger::log([sprintf($lang[$current_lang]["call_error_2"], $params['id'])]);

            return false;
        }
        \danog\MadelineProto\Logger::log([sprintf($lang[$current_lang]["call_confirming"], $this->calls[$params['id']]->getOtherID())], \danog\MadelineProto\Logger::VERBOSE);
    ...
    public function complete_call($params)
    {
        ...
        if ($this->call_status($params['id']) !== \danog\MadelineProto\VoIP::CALL_STATE_ACCEPTED || !isset($this->calls[$params['id']]->storage['b'])) {
            \danog\MadelineProto\Logger::log([sprintf($lang[$current_lang]["call_error_3"], $params['id'])]);

            return false;
        }
        \danog\MadelineProto\Logger::log([sprintf($lang[$current_lang]["call_completing"], $this->calls[$params['id']]->getOtherID())], \danog\MadelineProto\Logger::VERBOSE);

        $dh_config = $this->get_dh_config();
        if (hash('sha256', $params['g_a_or_b'], true) != $this->calls[$params['id']]->storage['g_a_hash']) {
            throw new \danog\MadelineProto\SecurityException($lang[$current_lang]["invalid_g_a"]);
        }
        ...
        if (substr(sha1($key, true), -8) != $params['key_fingerprint']) {
            throw new \danog\MadelineProto\SecurityException($lang[$current_lang]["fingerprint_invalid"]);
        }
        $visualization = [];
        $length = new \phpseclib\Math\BigInteger(count($this->emojis));
    ...
    public function discard_call($call, $reason, $rating = [], $need_debug = true)
    {
        ...
        \danog\MadelineProto\Logger::log([sprintf($lang[$current_lang]["call_discarding"], $call['id'])], \danog\MadelineProto\Logger::VERBOSE);
        ...
        if (!empty($rating)) {
            \danog\MadelineProto\Logger::log([sprintf($lang[$current_lang]["call_set_rating"], $call['id'])], \danog\MadelineProto\Logger::VERBOSE);
        ...
        if ($need_debug) {//} && isset($this->calls[$call['id']]->storage['not_modified'])) {
            \danog\MadelineProto\Logger::log([sprintf($lang[$current_lang]["call_debug_saving"], $call['id'])], \danog\MadelineProto\Logger::VERBOSE);
