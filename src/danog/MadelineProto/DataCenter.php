    ...
    */
    ...
    public function ___construct($dclist, $settings)
    {
        ...
        foreach ($this->sockets as $key => $socket) {
            if ($socket instanceof Connection) {
                \danog\MadelineProto\Logger::log([sprintf($lang[$current_lang]["dc_con_start"], $key)], \danog\MadelineProto\Logger::VERBOSE);
                $socket->old = true;
    ...
    public function dc_disconnect($dc_number)
    {
        ...
        if (isset($this->sockets[$dc_number])) {
            \danog\MadelineProto\Logger::log([sprintf($lang[$current_lang]["dc_con_stop"], $dc_number)], \danog\MadelineProto\Logger::VERBOSE);
    ...
    public function dc_connect($dc_number)
    {
        ...
        \danog\MadelineProto\Logger::log([sprintf($lang[$current_lang]["dc_con_test_start"], $dc_number, $test, $ipv6, $this->settings[$dc_config_number]['protocol'])], \danog\MadelineProto\Logger::VERBOSE);
