...
class RPCErrorException extends \Exception
{
    ...
    public static function make(
        ...
        return match ($rpc) {
            ...
            'AI_COMPOSE_TASK_MISSING' => new self($rpc, 'No AI task was specified. The caller must provide at least one of: proofread, translate (with a target language), tone, or emojify.', $code, $caller, $previous),
            'AICOMPOSE_FLOOD_PREMIUM' => new self($rpc, 'You\'ve reached the daily limit of AI text transformations, upgrade to [Telegram Premium](https://core.telegram.org/api/premium) to get **50x** times more AI text transformations per day!', $code, $caller, $previous),
            'AICOMPOSE_TONE_INVALID' => new self($rpc, 'The specified tone is invalid.', $code, $caller, $previous),
            'AICOMPOSE_TONE_TITLE_INVALID' => new self($rpc, 'The specified tone title is invalid.', $code, $caller, $previous),
            ...
            'BIRTHDAY_ALREADY' => new self($rpc, 'The target user already has a birthday set.', $code, $caller, $previous),
            ...
            'BOT_ID_INVALID' => new self($rpc, 'The specified bot ID is invalid.', $code, $caller, $previous),
            ...
            'CALL_NOT_ACTIVE' => new self($rpc, 'The specified call is not active.', $code, $caller, $previous),
            ...
            'COLLECTION_ID_INVALID' => new self($rpc, 'The specified collection ID is invalid.', $code, $caller, $previous),
            ...
            'CREDENTIAL_INVALID' => new self($rpc, 'The specified credential is invalid.', $code, $caller, $previous),
            ...
            'EFFECT_CHAT_INVALID' => new self($rpc, 'Message [effects](https://core.telegram.org/api/effects) can only be used in private 1-on-1 chats, but the caller tried to send a message with an effect to a group or channel.', $code, $caller, $previous),
            ...
            'EMAIL_INSTALL_MISSING' => new self($rpc, 'Attempting to send a code to the recovery email, but no email is configured.', $code, $caller, $previous),
            ...
            'ENTITY_DATE_FORMAT_INVALID' => new self($rpc, 'One of the passed messageEntityFormattedDate objects has an invalid format (i.e. an invalid combination of the format flags).', $code, $caller, $previous),
            'ENTITY_DATE_INVALID' => new self($rpc, 'One of the passed messageEntityFormattedDate objects has an invalid date: the allowed value ranges from `0` to the current date plus 1098 days (`time()+1098*86400`).', $code, $caller, $previous),
            'ENTITY_DATE_TOO_LONG' => new self($rpc, 'The maximum text span that can be covered by a date entity is 31 UTF-16 code units if any of the date formatting flags is set, or 127 UTF-16 code units without.  .', $code, $caller, $previous),
            ...
            'EXTENDED_MEDIA_EMPTY' => new self($rpc, 'The specified extended media is empty.', $code, $caller, $previous),
            'EXTENDED_MEDIA_INVALID' => new self($rpc, 'The specified paid media is invalid.', $code, $caller, $previous),
            'EXTENDED_MEDIA_PEER_INVALID' => new self($rpc, 'Paid media is not allowed for the target peer.', $code, $caller, $previous),
            ...
            'GROUPCALL_FORBIDDEN' => new self($rpc, 'The specified group call cannot be used in this context.', $code, $caller, $previous),
            ...
            'INPUT_STARS_AMOUNT_INVALID' => new self($rpc, 'The specified offer amount in stars is invalid, see [here &raquo;](https://core.telegram.org/api/gifts#collectible-gift-purchase-offers) for the allowed range.', $code, $caller, $previous),
            'INPUT_STARS_NANOS_INVALID' => new self($rpc, ' The specified offer amount in nanotons is invalid, see [here &raquo;](https://core.telegram.org/api/gifts#collectible-gift-purchase-offers) for the allowed range.', $code, $caller, $previous),
            ...
            'LIMIT_PER_POST_INVALID' => new self($rpc, 'The specified reactions_limit value is invalid.', $code, $caller, $previous),
            ...
            'MANAGER_INVALID' => new self($rpc, 'The specified manager bot is invalid.', $code, $caller, $previous),
            'MANAGER_PERMISSION_MISSING' => new self($rpc, 'The specified manager bot does not have the [user](https://core.telegram.org/constructor/user).`bot_can_manage_bots` flag set.', $code, $caller, $previous),
            ...
            'MESSAGE_REQUIRED' => new self($rpc, 'A non-empty list of IDs must be passed to `id`.', $code, $caller, $previous),
            ...
            'MSG_VOICE_TOO_LONG' => new self($rpc, 'The specified voice message is too long to be transcribed.', $code, $caller, $previous),
            ...
            'NAME_INVALID' => new self($rpc, 'The specified bot name is invalid.', $code, $caller, $previous),
            'NEED_ACTION_MISSING' => new self($rpc, 'The caller didn\'t specify a valid action (either save or suggest) for the contact profile photo upload.', $code, $caller, $previous),
            ...
            'PASSKEY_ORIGIN_MISMATCH' => new self($rpc, 'Third-party clients currently don\'t support passkeys even when changing the origin.', $code, $caller, $previous),
            ...
            'PINNED_TOPIC_NOT_MODIFIED' => new self($rpc, 'The specified topic is already pinned.', $code, $caller, $previous),
            ...
            'PREMIUM_PURPOSE_INVALID' => new self($rpc, 'The specified InputStorePaymentPurpose is invalid.', $code, $caller, $previous),
            ...
            'PUBLIC_BROADCAST_EXPECTED' => new self($rpc, '`channel` only accepts a channel, but a supergroup was passed.', $code, $caller, $previous),
            'PUBLIC_KEY_INVALID' => new self($rpc, 'The specified e2e public key is invalid.', $code, $caller, $previous),
            ...
            'RANDOM_ID_DUPLICATE' => new self($rpc, 'You provided a random ID that was already used.', $code, $caller, $previous),
            ...
            'REQUEST_MSG_EXPIRED' => new self($rpc, 'The request specified in request_msg_id has already expired.', $code, $caller, $previous),
            'REQUEST_TOKEN_INVALID' => new \danog\MadelineProto\RPCError\RequestTokenInvalidError($code, $caller, $previous),
            'RESELL_STARS_TOO_FEW' => new self($rpc, 'The offered price is too low.', $code, $caller, $previous),
            'RESELL_STARS_TOO_MUCH' => new self($rpc, 'The offered price is too high.', $code, $caller, $previous),
            ...
            'STARGIFT_ATTRIBUTE_INVALID' => new self($rpc, 'One of the specified star gift attributes is invalid.', $code, $caller, $previous),
            'STARGIFT_INVALID' => new self($rpc, 'The passed gift is invalid.', $code, $caller, $previous),
            'STARGIFT_MESSAGE_INVALID' => new self($rpc, 'The specified inputInvoiceStarGift.message is invalid.', $code, $caller, $previous),
            'STARGIFT_NOT_FOUND' => new self($rpc, 'The specified gift was not found.', $code, $caller, $previous),
            'STARGIFT_NOT_OWNER' => new self($rpc, 'You\'re not the owner of the gift you trying to transfer.', $code, $caller, $previous),
            'STARGIFT_NOT_UNIQUE' => new self($rpc, 'You can\'t transfer a non-collectible gift.', $code, $caller, $previous),
            'STARGIFT_OBJECT_INVALID' => new self($rpc, 'The specified star gift object is invalid.', $code, $caller, $previous),
            'STARGIFT_OFFER_INVALID' => new self($rpc, 'The specified offer amount is invalid.', $code, $caller, $previous),
            'STARGIFT_OFFER_NOT_ALLOWED' => new self($rpc, 'You can\'t send a purchase offer for this gift.', $code, $caller, $previous),
            ...
            'TEXTDRAFT_PEER_INVALID' => new self($rpc, 'sendMessageTextDraftAction can only be used in private 1-on-1 chats.', $code, $caller, $previous),
            ...
            'TODO_ITEMS_TOO_MUCH' => new \danog\MadelineProto\RPCError\TodoItemsTooMuchError($code, $caller, $previous),
            ...
            'URL_EXPIRED' => new self($rpc, 'The specified OAuth request has expired.', $code, $caller, $previous),
            ...
            'USER_BOT_TO_BOT_DISABLED' => new \danog\MadelineProto\RPCError\UserBotToBotDisabledError($code, $caller, $previous),
            ...
            'USERNAME_SUFFIX_MISSING' => new self($rpc, 'The required `bot` suffix is missing from the passed username.', $code, $caller, $previous),
            ...
            'VIDEO_DURATION_INVALID' => new self($rpc, 'The duration of the specified video is invalid.', $code, $caller, $previous),
            ...
            'WEBAPP_REQ_ID_INVALID' => new self($rpc, 'The specified webapp_req_id is invalid.', $code, $caller, $previous),
            'WEBAUTH_TOKEN_EXPIRED' => new self($rpc, 'The specified auth token has expired.', $code, $caller, $previous),
            ...
            'ACCESS_DENIED' => new self($rpc, 'The account was deactivated, or is a bot/service account.', $code, $caller, $previous),
            ...
            'BOT_FORUM_CREATE_FORBIDDEN' => new self($rpc, 'Since the bot\'s user.bot_forum_can_manage_topics flag is **not** set, the user cannot create or modify bot forum topics.', $code, $caller, $previous),
            'BOT_GUARD_NOT_SUPPORTED' => new self($rpc, 'This bot is not designated as a "join guard" bot. This method is only available to bots that mediate user joins to chats. .', $code, $caller, $previous),
            ...
            'GROUPCALL_CHANGE_FORBIDDEN' => new self($rpc, 'You cannot change this group call setting.', $code, $caller, $previous),
            ...
            'USER_DISALLOWED_STARGIFTS' => new self($rpc, 'The recipient user has configured restrictions on which categories of star gifts they\'re willing to accept (unique, limited, or unlimited): the sender attempted to get a payment form for a gift that falls into a category the recipient has blocked.', $code, $caller, $previous),
            ...
            'EDIT_MESSAGE_TEMP_RESTRICTED' => new self($rpc, 'Message editing is temporarily forbidden for this user due to regulatory restrictions.', $code, $caller, $previous),
            ...
            'POLL_COUNTRY_RESTRICTED' => new self($rpc, 'Users from the current user\'s country cannot vote in this [country-restricted poll &raquo;](https://core.telegram.org/api/poll#country-restricted-polls).', $code, $caller, $previous),
            'POLL_MEMBER_RESTRICTED' => new self($rpc, 'Only channel subscribers can vote in this poll.', $code, $caller, $previous),
